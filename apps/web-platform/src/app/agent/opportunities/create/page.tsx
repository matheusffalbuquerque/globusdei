...
import { ArrowLeft, AlertCircle, Send, Lock } from 'lucide-react';
...
export default function AgentCreateOpportunityPage() {
  ...
  // Carrega empreendimentos do agente para vincular
  useEffect(() => {
    if (!session) return;
    apiFetch('/empreendimentos/mine', { session: session as AppSession })
      .then((data: Empreendimento[]) => {
        ...
        if (data.length > 0) {
          setForm((f) => ({ ...f, empreendimentoId: data[0].id }));
        }
      })
      .catch(() => setEmpreendimentos([]));
  }, [session]);

  // Agente precisa ser APPROVED
    ...
    return (
      <div className="mx-auto max-w-lg space-y-4 py-10 text-center">
        ...
        <p className="text-sm text-muted-foreground">
          Somente agentes aprovados com iniciativas cadastradas podem publicar oportunidades.
          Conclua seu processo de qualificação primeiro.
        </p>
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError(null);
    try {
      const created = await apiFetch('/opportunities/agent', {
        method: 'POST',
      ...
      router.push(`/agent/opportunities/${created.id}`);
    } catch (err) {
    ...
    <div className="mx-auto max-w-2xl space-y-6">
      ...
      <div>
        <p className="text-xs font-semibold uppercase tracking-widest text-muted-foreground">
          Minha iniciativa · Nova publicação
        </p>
        <h1 className="text-2xl font-bold tracking-tight text-foreground">
          Publicar oportunidade
        </h1>
        <p className="mt-1 text-sm text-muted-foreground">
          Compartilhe uma vaga ou chamada vinculada à sua iniciativa.
        </p>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Apenas título e descrição são obrigatórios. Os demais campos enriquecem a publicação.
          </CardDescription>
        </CardHeader>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            {empreendimentos.length > 0 && (
              <div className="space-y-1.5">
                ...
                <Select
                  ...
                  onChange={(e) =>
                    setForm((f) => ({ ...f, empreendimentoId: e.target.value }))
                  }
                >
                  <option value="">Nenhuma (publicar como agente)</option>
            ...
            <div className="space-y-1.5">
              ...
              <Textarea
                required
                rows={8}
                placeholder="Descreva a oportunidade, o que espera dos candidatos e como entrar em contato…"
