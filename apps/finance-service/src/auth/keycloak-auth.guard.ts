...
export class KeycloakAuthGuard implements CanActivate {
  ...
  private getDevelopmentUser(headers: Record<string, string | string[] | undefined>) {
    ...
    const sub = getValue(headers['x-dev-user-sub']);
    const email = getValue(headers['x-dev-user-email']);
    const name = getValue(headers['x-dev-user-name']);
    const rolesHeader = getValue(headers['x-dev-user-roles']);
