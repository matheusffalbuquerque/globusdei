...
/**
 * Validates Keycloak bearer tokens and supports local development headers.
 */
@Injectable()
export class KeycloakAuthGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    if (authorization?.startsWith('Bearer ')) {
      request.user = await this.verifyToken(authorization.substring('Bearer '.length));
      return true;
    }

    const internalToken = this.getHeaderValue(request.headers['x-internal-service-token']);
    if (internalToken && this.internalToken && internalToken === this.internalToken) {
      request.user = {
  ...
  private getDevelopmentUser(headers: Record<string, string | string[] | undefined>) {
